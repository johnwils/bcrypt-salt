...
class BcryptSalt {
   ...
   */
  constructor(args) {
    this.maxHashTime = args && typeof args.maxHashTime === 'number' && args.maxHashTime > 0 ? args.maxHashTime : 1000;
  ...
  endLog() {
    this.logs && console.log(`\nReccomended bcrypt saltRounds for this hardware is ${this.saltRounds} running in ${this.hashTime}ms.\n1 higher exceeds max hash time (${this.maxHashTime}ms)\n`);
  }

  run() {
    bcrypt.hashSync(this.password, this.nextSaltRounds);
    this.nextHashTime = now() - this.timeBefore;
